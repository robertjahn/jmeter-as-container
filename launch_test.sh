...
# Usage:
# ./run_test.sh testscript.jmx result.jtl yourserverip [MyLoadTestName]
...
sudo rm -f -r $2
sudo mkdir $2
sudo docker run --name jmeter-test -v "${PWD}/scripts":/scripts -v "${PWD}/$2":/results --rm -d jmeter ./jmeter/bin/jmeter.sh -n -t /scripts/$1 -e -o /results -l result.tlf -JSERVER_UR="$3" -JDT_LTN="$DT_LTN"
